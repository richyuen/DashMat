...
- For same-page live-apply flows, do not assume moving restore/bootstrap compute into the existing load/session-apply callback is a win; if `click -> live-apply commit` regresses enough to erase the `live-apply commit -> ready` gain, roll it back even when request count drops.
