...
- For PortOpt warm switch, evaluate `readyMedian`, `restoredTabReadyMedian`, and `weightsReadyMedian` together. A March 15, 2026 page-load-control no-op probe improved restored/weights but regressed ready badly enough that it was not worth landing.
- Shared modal unmounting can move different page metrics in opposite directions. A March 16, 2026 raw-DB-modal `keepMounted=False` probe improved Analytics and PortOpt restored/weights but regressed PortOpt `readyMedian` and Regression, so treat modal-unmount timing changes as explicit tradeoffs rather than automatic wins.
