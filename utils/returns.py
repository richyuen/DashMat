...
@cache_config.cache.memoize(timeout=0)
def calculate_rolling_returns(raw_data, periodicity, selected_series, returns_type, benchmark_assignments, long_short_assignments, date_range, rolling_window="1y", rolling_return_type="annualized", rolling_metric="total_return", vol_scaler: float = 0, vol_scaling_assignments: str = "", risk_free_returns_json: str = "", use_risk_free: bool = True):
        ...
        for series in selected_series:
            ...
            if rolling_metric == "total_return":
                ...
                if risk_free_series is not None:
                    func = lambda x: sharpe_ratio_with_risk_free(x, periodicity, periods_per_year, risk_free_series)
                else:
                ...
                if risk_free_series is not None:
                    func = lambda x: sortino_ratio_with_risk_free(x, periodicity, periods_per_year, risk_free_series)
                else:
